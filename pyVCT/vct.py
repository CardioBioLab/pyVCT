...
from pyVCT.hamiltonian import MAX_FOCALS, calcdH
...
class VCT:
    def __init__(
        ...
        fibers_dist: float = 0.01,
    ) -> None:
        """
        Initiliaze Virual Cardiac Monolayer class

            Params:
                tissue_type: str, type of tissue, by deafault provided 'neonatal_rat'. If you use custom config provide your tissue name
                scenario: str, one of possible simulation modes. One of 'monolayer' and 'single'
        ...
        # if no params provided load params for nennatal rat monolayer
        if path2params and tissue_type:
        ...
        if fibers:
            self.energy_config = self.energy_config["fiber"]
        else:
            self.energy_config = self.energy_config["no_fiber"]
        ...
        # init fibers distance
        self.fibers_distance = fibers_dist
    ...
    def _init_constants(self):
        ...
        # define size of area
        self.marginx, self.marginy = int(self.sizeX / 10 / self.voxsize), int(
            self.sizeY / 10 / self.voxsize
        )
    ...
    def _cast_units(self):
        """
        Cast units acoording to voxsize
        """
        ...
        self.energy_config["TARGETVOLUME_CM"] = (
            self.energy_config["TARGETVOLUME_CM"] / 1000 / self.voxsize / self.voxsize
        )
        self.energy_config["TARGETVOLUME_FB"] = (
            self.energy_config["TARGETVOLUME_FB"] / 1000 / self.voxsize / self.voxsize
        )
        self.energy_config["INELASTICITY_CM"] = self.energy_config[
            "INELASTICITY_CM"
        ] * (scale**4)
        self.energy_config["INELASTICITY_FB"] = self.energy_config[
            "INELASTICITY_FB"
        ] * (scale**4)
        ...
        self.energy_config["LMAX_CM"] = (
            self.energy_config["LMAX_CM"] / 1000 / self.voxsize
        )
        self.energy_config["LMAX_FB"] = (
            self.energy_config["LMAX_FB"] / 1000 / self.voxsize
        )

    def _init_cells(self):
        """
        Initialize arrays for cells
        """
        ...
        self.cell_sizes = (
            np.ones((self.ncx * self.ncy + 1), dtype=int) * (2 * self.r + 1) ** 2
        )
        ...
        assert (
            dx > 2 * self.r and dy > 2 * self.r
        ), "Too dense, please decrease number of cells"

        cell_number = 0
        for i in range(self.ncx):
            for j in range(self.ncy):

                dvx = np.random.randint(0, 2**32 - 1) % int(dx - 2 * self.r + 1) - (
                    dx / 2 - self.r
                )
                dvy = np.random.randint(0, 2**32 - 1) % int(dx - 2 * self.r + 1) - (
                    dx / 2 - self.r
                )
    ...
    def _init_fibers(self, fibers):
        self.fibers_distance = int(self.fibers_distance / self.voxsize)
    ...
    def step(self):
        ...
        for _ in range(self.nvx * self.nvy):
            ...
            if (
                self.types[ttag] * self.types[stag] == 1
                and self.contacts[x_target, y_target]
                * self.contacts[x_source, y_source]
                == 1
    ...
    def simulate(self, n_steps):
        """
        Simulates CPM model for n_steps

        Params:
            n_steps: int, number of simulation steps

        Return:
            acceptance_rate: float, fraction of accepted Monte-Carlo steps
        Note:
            1 simulation step contains nvx*nvy Monte-Carlo steps
        """

        for i in tqdm(range(n_steps)):
            self.step()
        acceptance_rate = self.moves / self.tries
    ...
    def move(self, x_target, y_target, x_source, y_source):
        ...
        if (
            stag
            and (not self.contacts[x_target, y_target])
            and self.attached[stag] < self.energy_config[MAX_FOCALS(self.types[stag])]
        ):

...
def calcdH(ctags, types, fibers, contacts, CMs, bond, csize, xt, yt, xs, ys, params):
    ...
    dH = dHcontact + dHvol + dHnuclei + dHfocals + dHsyncytium
    return dH
...
def calcdHcontact(ctags, types, xt, yt, xs, ys, params):
    ...
    for n in range(8):
        nbtag = ctags[nbs[n][0]][nbs[n][1]]
        Hcontact += contactenergy(
            ctags[xt][yt], nbtag, types[ctags[xt][yt]], types[nbtag], params
        )
        Hcontactn += contactenergy(
            ctags[xs][ys], nbtag, types[ctags[xs][ys]], types[nbtag], params
        )
...
def calcdHprotrude(ctags, types, contacts, CMs, xt, yt, xs, ys, Qt, Qs, params):
    ...
    if Qs and Qt:
        ...
        coss = np.cos(-np.arctan((xs - CMs[stag][0]) / (ys - CMs[stag][1])))

        cost = np.cos(-np.arctan((xt - CMs[stag][0]) / (yt - CMs[stag][1])))
    ...
    if contacts[xs][ys]:
        ...
        if distt < params[LMAX(types[stag])]:
            if dists < params[LMAX(types[stag])]:
                dH = params[GN(types[stag])] * (
                    (1 / distt) * abs(1 / cost) - (1 / dists) * abs(1 / coss)
                )
            else:

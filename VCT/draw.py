...
def make_image(n_int, CMs_ind):
    ...
    if (
        sys.argv[3] != "0"
    ):  # if not 0, show contacts/attachments at least on the edge of the cell
        f = 0.5
    if (
        sys.argv[3] == "2"
    ):  # if 2, show all of the attachment sites, even those under the cell
        cont_edges = 1
    ...
    img[:, :, 0] = (
        CMs * (1 - 0.5 * edges) * 255
        + FBs * (1 - 0.5 * edges) * 255
        + f * conts * cont_edges * CMs * 255
        + f * conts * cont_edges * FBs * 255
        + Fibs
    )
    img[:, :, 1] = (
        FBs * (1 - 0.5 * edges) * 255
        + 2 * f * conts * cont_edges * CMs * 255
        + f * conts * cont_edges * FBs * 255
        + Fibs
    )
    img[:, :, 2] = (
        FBs * (1 - 0.5 * edges) * 255 - f * conts * cont_edges * FBs * 255 + Fibs
    )
...
print(
    ...
    len(FBs_ind),
    "; % FBs:",
    1.0 * len(FBs_ind) / (len(CMs_ind) + len(FBs_ind)),
)
